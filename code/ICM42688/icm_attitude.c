...
/* 全字段零初始化；q0=1.0f（单位四元数）由 icm_attitude_init() 显式设置。
 * 选用 {0} 而非按字段列出，避免之后向 icm_attitude_state_t 加新成员时
 * 因为初始化列表与结构体字段顺序错位而引入静默 bug。 */
static volatile icm_attitude_state_t g_icm_attitude = {0};
                                         ...
                                         float dt_s)
{
    ...
    g_icm_attitude.gyro_bias_flash_synced = 0U;
}
...
uint8 icm_attitude_save_gyro_bias_to_flash(void)
{
    ...
    g_icm_attitude.gyro_bias_flash_synced = 1U;
    return 1U;
}
...
uint8 icm_attitude_is_gyro_bias_flash_synced(void)
{
    return g_icm_attitude.gyro_bias_flash_synced;
}
